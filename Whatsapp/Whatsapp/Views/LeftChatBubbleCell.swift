...
class LeftChatBubbleCell: UICollectionViewCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        LeftChatBubbleCell.totalWidth = self.contentView.frame.width * 0.80 - SizeForBubble.RightInsetForCell
        addRoundToContainer()
    }
    ...
    func check() -> CGFloat {
        ...
        let size = CGSize(width: LeftChatBubbleCell.totalWidth - 5, height: 1000)
        ...
        let sizeCalulated  = String(messageTitle.text ?? "").boundingRect(with: size, options: options, attributes: attributes, context: nil)
            ...
            let diff = LeftChatBubbleCell.totalWidth - sizeCalulated.width
            addRoundToContainer()
